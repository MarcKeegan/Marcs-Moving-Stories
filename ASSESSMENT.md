...
1. **POI-aware stories** — the server already exposes `/api/nearby-pois` (Places Nearby Search) and the iOS app uses it, but the web client doesn't. Feeding 2–3 real landmarks per segment into the prompt would ground stories in the actual route — the biggest differentiation win available, mostly using code that already exists.
